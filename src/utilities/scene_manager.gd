...
signal content_finished_loading(content)  ## internal - triggered when content is loaded and final data handoff and transition out begins
signal content_invalid(content_path: String)  ## internal - triggered when attempting to load invalid content (e.g. an asset does not exist or path is incorrect)
signal content_failed_to_load(content_path: String)  ## internal - triggered when loading has started but failed to complete

var _loading_screen_scene: PackedScene = preload("res://src/utilities/loading_screen.tscn")  ## reference to loading screen PackedScene
...
func swap_scenes(
	...
	transition_type: String = "fade_to_black"
) -> void:
...
func swap_scenes_zelda(
	scene_to_load: String, load_into: Node, scene_to_unload: Node, move_dir: Vector2
) -> void:
	if _loading_in_progress:
		push_warning("SceneManager is already loading something")
		return
...
#	This block is only used by the zelda transition, which is a special case that doesn't use the loading screen
	if _transition == "zelda":
		...
		tween_in.tween_property(incoming_scene, "position", Vector2.ZERO, 1).set_trans(
			Tween.TRANS_SINE
		)
